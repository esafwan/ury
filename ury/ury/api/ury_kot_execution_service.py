...
def _transition(kot, target_state, idempotency_key, actor_field, timestamp_field, event, manager_override=False):
	...
	if locked:
		...
		# the latest committed row -- even though it runs after `_lock_execution_row`
		# took `FOR UPDATE` on this same row. Overwrite `audit_log` with the
		# value the locking read just fetched so the read-modify-write append
		# below lands on top of the latest committed entries instead of
		# silently dropping a concurrently committed one.
