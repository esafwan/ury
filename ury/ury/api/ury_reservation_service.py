...
def _resolve_group_rows(reservation_name):
	...
	could silently clobber the other's `to_status`. `FOR UPDATE` serializes
	concurrent callers on this group the same way `_lock_bin_row` and the
	other `FOR UPDATE` sites in this module do, and stays on this request's
