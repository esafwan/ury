...
from grillax.grillax.hooks.user_access_utils import (
    getBranch
)
...
def checklist():
    ...
    pos_opening_list = frappe.get_all(
        "POS Opening Entry",
        filters={"branch": branchName,"docstatus":1,"status":"Open"},
        fields={"posting_date"}
    )
    ...
    # todo: get value using parent
    pos_profile_name = frappe.get_value("POS Profile",{"branch":branchName},"name")
    pos_profile = frappe.get_doc("POS Profile",pos_profile_name)
    dependent_checklist = pos_profile.custom_dependent_checklist

    # if no checklist in pos profile then return completed
    ...
    # if no checklist is provided for the user, then return
